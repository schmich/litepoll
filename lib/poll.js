var db = require('./settings').settings.mongo;
var co = require('co');
var mongoose = require('mongoose');
var increment = require('mongoose-auto-increment');
var encoding = require('./encoding');
var Promise = require('bluebird');
var NotFoundError = require('./not-found');
 
increment.initialize(db);

var commentSchema = mongoose.Schema({
  ip: String,
  text: String
}, { _id: false });

var pollSchema = mongoose.Schema({
  title: String,
  opts: [String],
  votes: [Number],
  strict: Boolean,
  creator: String,
  comments: [commentSchema]
});

pollSchema.plugin(increment.plugin, {
  model: 'Poll',
  startAt: 1000
});

var Poll = mongoose.model('Poll', pollSchema);
Promise.promisifyAll(Poll);

Poll.create = function(opts) {
  var poll = new Poll(opts);
  return new Promise(function(resolve, reject) {
    poll.save(function() {
      resolve(poll);
    });
  });
};

Poll.find = function(id) {
  return this.findOneAsync({ _id: id }).then(function(poll) {
    if (poll) {
      return Promise.resolve(poll);
    } else {
      return Promise.reject(new NotFoundError());
    }
  });
};

Poll.findEncoded = function(encodedId) {
  var id = encoding.toNumber(encodedId);
  if (isNaN(id)) {
    return Promise.reject(new NotFoundError());
  }

  return this.find(id);
};

Poll.vote = function(id, optionIndex) {
  var update = { $inc: { } };
  update['$inc']['votes.' + optionIndex] = 1;

  return Poll.findOneAndUpdateAsync({ _id:  id }, update, {}).then(function(poll) {
    if (poll) {
      return Promise.resolve(poll);
    } else {
      return Promise.reject(new NotFoundError());
    }
  });
};

Poll.addComment = function(id, ip, comment) {
  return co(function *() {
    var update = { $push: { comments: { text: comment, ip: ip } } };
    var select = { select: '_id' };
    var result;

    var poll = yield Poll.find(id);
    if (poll.strict) {
      var find = { _id: id, 'comments.ip': { $ne: ip } };
      result = yield Poll.findOneAndUpdateAsync(find, update, select);
    } else {
      var find = { _id: id };
      result = yield Poll.findOneAndUpdateAsync(find, update, select);
    }

    return result ? true : false;
  });
};

module.exports = Poll;
